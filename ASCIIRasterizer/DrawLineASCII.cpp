...
namespace DrawLineASCII
{
	...
	void drawLineVertical(int x0, int y0, int height, bool up=false)
	{
		...
		if(up)
		{
			for(int y{ 0 }; y < height; ++y)
			{
				Screen::draw(x0, y0 - y, Slope::patterns[1][0][0]);
			}
		}
		else
		{
	...
	void drawLineVerticalSloped(int x0, int y0, int height, bool up=false)
	{
		...
		if(up) 
		{
			...
			Screen::draw(x0, y0 - (height - 1), '.'); //Use the dot when going up
		}
		else
		{
			for(int y{ 0 }; y < height - 1; ++y)
			...
			Screen::draw(x0, y0 + (height - 1), Slope::patterns[1][Slope::HIGHER_SLOPE_DY - 1][0]);
		}
	}
	...
	void drawSlopeOnce(SlopeEnum slope, int x0, int y0, bool up)
	{
		...
		{ 
			for(int y{ 0 }; y < dy; ++y)
				for(int x{ 0 }; x < dx; ++x)
				{
					Screen::draw(x0 + x, y0 + y, Slope::patterns[slope][y][x]);
				}
		}
	}
